...
### 数据口径
1. **CPI/PPI 口径未确认**(最高优先级):FRED 存**指数水平值** vs akshare 存**变化率**。同一指标两个源数值含义不同,`meta_indicator` 获信源可切换,但**当前默认获信源取哪个、下游分析是否知道口径差异,未定案**。影响所有分析/导出。
...
### 功能缺口
4. **GUI「全部同步」一键按钮未做**:全量同步目前靠脚本(`sync_engine.run_all`)或手动逐源。
5. **指标管理中心每行未显示数据截止日**(GUI 增强)。
6. **AI 模型切换未落地**:本地 ollama 用 deepseek-r1:14b;用户希望切 qwen2.5:7b(改 `default.yaml` 一行),未执行。

### 工程 / 运维观察
7. **后台长任务被系统 kill**:本次 2005-2022 回溯期间,后台任务先后 3 次被终止(运行 1h48m / 1.5m / 后又续传)。断点续传设计(upsert 幂等)兜住了,但**根因未排查**(超时?内存?系统策略?)。长期批量任务可靠性值得关注。
...
### 已修复、待真机验证
12. **GUI 表格卡顿修复(7fd6a87)** 与 **同步防卡(28fdd3f)**:代码级已修复并提交,但**用户在真机上拖动/同步验证**尚未确认。
...
python -m pytest tests/ -q               # 测试(209 全绿,~21s)
...
- 架构层面的可讨论点(供发散):全 TEXT 存储的取舍、SQLite 单写者上限、多源异构(FRED 水平值 vs akshare 变化率)的口径归一策略、指标获信源的自动化、长任务的断点续传与任务编排、GUI 线程模型、测试覆盖缺口(目前 209 个测试主要覆盖 importer/api/sync_engine/fred,对 GUI 与并发路径覆盖少)。
